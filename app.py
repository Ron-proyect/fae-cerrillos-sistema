...
# Columnas y orden EXACTOS que espera la Carga Masiva (sección 2 de la barra lateral).
# Se usan tanto para la plantilla base descargable como para la descarga de la Matriz Completa,
# de modo que ambos archivos tengan siempre el mismo formato y se puedan volver a subir tal cual.
COLUMNAS_MATRIZ_MAESTRA = [
    "Caso", "RIT", "Profesional", "Fecha Ingreso",
    "codnino", "fechanacimiento", "Nacionalidad", "CalidadJuridica",
    "DireccionNino", "Comuna", "Tribunal", "ConQuienVive"
] + NOMBRES_TABLA
...
def convertir_a_excel_completo(df_casos_actuales, df_entregas_total):
    ...
    que espera la Carga Masiva (sección 2 de la barra lateral), para poder editarlo y
    volver a subirlo directamente."""
    ...
    for col in COLUMNAS_MATRIZ_MAESTRA:
        if col not in df_final.columns:
            df_final[col] = None

    df_final = df_final.reindex(columns=COLUMNAS_MATRIZ_MAESTRA)

    for col in df_final.columns:
        if col in ("Fecha Ingreso", "fechanacimiento") or col in NOMBRES_TABLA:
            df_final[col] = pd.to_datetime(df_final[col], errors='coerce').dt.strftime('%d-%m-%Y').replace("NaT", "")

    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df_final.to_excel(writer, index=False, sheet_name='Matriz_Maestra')
    return output.getvalue()

def generar_plantilla_base():
    """Genera una planilla vacía (solo encabezados) con las columnas exactas que
    espera la Carga Masiva, para que el admin la llene desde cero sin errores de formato."""
    output = io.BytesIO()
    df_plantilla = pd.DataFrame(columns=COLUMNAS_MATRIZ_MAESTRA)
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df_plantilla.to_excel(writer, index=False, sheet_name='Matriz_Maestra')
    return output.getvalue()
...
if st.session_state.user_role == "admin":
    ...
    with st.sidebar.expander("1. 📝 Registrar Nuevo Caso", expanded=False):
        with st.form("nuevo_caso", clear_on_submit=True):
            ...
            # Se guarda el ID estable de la dupla (ej. "Dupla 3"), no el nombre de los integrantes,
            # para que renombrar integrantes más adelante no desvincule este caso de su dupla.
            prof = prof_elegido_form.split(":")[0].strip()
            ...
            if st.form_submit_button("Guardar Caso") and n_caso:
                try:
                    ...
                    st.success(f"✅ Caso {n_caso} guardado en la nube")
                    st.rerun()
                except Exception as e:
    ...
    with st.sidebar.expander("2. 🚀 Carga Masiva (Matriz Maestra)", expanded=False):
        st.download_button(
            ...
            "Plantilla_Matriz_Maestra_FAE.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            help="Descarga una planilla vacía con las columnas exactas que espera esta carga masiva."
        )
        st.caption("Llena esta plantilla y súbela abajo. En 'Profesional' puedes usar el ID de la dupla (ej. 'Dupla 3') o el nombre de sus integrantes.")
...
def obtener_dupla_id_para_caso(prof_en_caso):
    ...
    # 1) Coincidencia directa por ID de dupla (ej. "dupla 3") o por nombre de integrantes actual.
    for d_id, integrantes in duplas_nombres.items():
        if p_clean == integrantes.strip().lower() or p_clean == d_id.lower():
            return d_id
    # 2) Coincidencia por primer nombre (para datos antiguos cargados antes de este cambio).
    for d_id, integrantes in duplas_nombres.items():
        primer_nombre = integrantes.split("-")[0].split()[0].lower()
        if primer_nombre in p_clean:
            return d_id
    # 3) Coincidencia contra el mapeo inicial por defecto (por si el nombre nunca fue editado).
            ...
            with st.form("registrar_envio", clear_on_submit=True):
                ...
                if st.form_submit_button("Registrar Envío"):
                    try:
                        ...
                        st.success("✅ Envío registrado en la nube")
                        st.rerun()
                    except Exception as e:
                        st.error(f"❌ Error: {e}")
        else:
            st.caption("No hay casos disponibles para registrar envíos.")

if st.session_state.user_role == "admin":
    with st.sidebar.expander("4. 🗑️ Eliminar Caso", expanded=False):
        if not df_c_sidebar.empty:
            ...
            if st.button("Eliminar permanentemente"):
                if caso_a_borrar != "---":
                    try:
                        ...
                        st.warning(f"Caso '{caso_a_borrar}' eliminado de la nube.")
                        st.rerun()
                    except Exception as e:
