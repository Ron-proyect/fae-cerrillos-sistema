...
def login_screen():
    ...
    with col2:
        ...
        with st.container(border=True):
            st.markdown(f"<h2 style='text-align: center; color: {COLOR\\\_VERDE\\\_IRIDEM};'>Acceso al Sistema</h2>", unsafe_allow_html=True)
...
if not df_c.empty:
    ...
    st.markdown(f"<h3 style='text-align: center; color: {COLOR\\\_GRIS\\\_PIZARRA}; margin-top: 0;'>Control de Plazos y Automatización Institucional</h3>", unsafe_allow_html=True)
    st.divider()
    ...
    with tab_ind:
        ...
        if not df_c_filtrado.empty:
            ...
            with col_graf2:
                ...
                st.write(f"<div style='margin-top: 10px; text-align: center;'>Total: {cumple_count + no_cumple_count} casos</div>", unsafe_allow_html=True)
            ...
            nombre_display = f"📍 {caso_sel}" if es_seleccionado_grafico else caso_sel
 
            st.markdown(f"""<div class="{clase\\\_banner}">Caso: {nombre_display} | RIT: {rit_c} | Ingreso: {f_ingreso.strftime('%d-%m-%Y')} | Antigüedad: {m_ant_tit} meses</div>""", unsafe_allow_html=True)
                ...
                st.markdown(f"""
                    <div class="case-info-banner">
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                            <div>
                                🆔 Cod. Niño: {info_c.get('codnino', 'S/I')}<br>
                                👤 Caso: {info_c['Caso']}<br>
                                🎂 F. Nacimiento: {pd.to_datetime(info_c.get('fechanacimiento'), errors='coerce').strftime('%d-%m-%Y') if info_c.get('fechanacimiento') != 'S/I' else 'S/I'}<br>
                                🎂 Edad: {info_c['Edad']} años<br>
                                ⚖️ Calidad Jurídica: {info_c.get('CalidadJuridica', 'S/I')}<br>
                                🏠 Con Quién Vive: {info_c.get('ConQuienVive', 'S/I')}
                            </div>
                            <div>
                                📄 Causa RIT: {info_c['RIT']}<br>
                                🏛️ Tribunal: {info_c.get('Tribunal', 'S/I')}<br>
                                📍 Comuna: {info_c.get('Comuna', 'S/I')}<br>
                                🗺️ Dirección: {info_c.get('DireccionNino', 'S/I')}<br>
                                🤝 Profesional / Dupla: {info_c['Profesional']}<br>
                                ⏱️ Antigüedad: {info_c['Meses']} meses | Próx. Inf: {info_c['Próximo Informe']} ({info_c['F. Límite (Teo)']})
                            </div>
                        </div>
                    </div>
                """, unsafe_allow_html=True)
